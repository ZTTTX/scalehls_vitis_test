...
def runKernel(opt):
    time_used = 0
    torch.manual_seed(0)
    t_config = parse_params_header("/mnt/shared/home/tz32/scalehls_vitis_test/src/LeNet_test.hpp")
    ...
    opt.DATA_SIZE = 48000#max(len(inputs_list), len(weights), len(biases))
    ...
    boHandle5 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(4))
    boHandle6 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(5))
    boHandle7 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(6))
    boHandle8 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(7))
    boHandle9 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(8))
    boHandle10 = pyxrt.bo(d, opt.DATA_SIZE * 4, pyxrt.bo.normal, kHandle.group_id(9))
    ...
    zero_float = struct.pack('f', 0.001)
    ...
    for i in range(opt.DATA_SIZE):
        ...
        bo5[i * 4: (i + 1) * 4] = zero_float
        bo6[i * 4: (i + 1) * 4] = zero_float
        bo7[i * 4: (i + 1) * 4] = zero_float
        bo8[i * 4: (i + 1) * 4] = zero_float
        bo9[i * 4: (i + 1) * 4] = zero_float
        bo10[i * 4: (i + 1) * 4] = zero_float
    ...
    boHandle5.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)
    boHandle6.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)
    boHandle7.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)
    boHandle8.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)
    boHandle9.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)
    boHandle10.sync(pyxrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE, opt.DATA_SIZE * 4, 0)

    print("Start the kernel")
    
    start_time = time.time()
    
    run = kHandle(boHandle1, boHandle2, boHandle3, boHandle4,boHandle5,boHandle6,boHandle7,boHandle8,boHandle9,boHandle10, opt.DATA_SIZE)
    state = run.wait()
